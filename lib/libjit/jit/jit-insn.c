...
jit_value_t jit_insn_tail_call( jit_function_t func, const char *name, jit_value_t *args, unsigned int num_args, jit_label_t* entry_point ) {
	...
	/* Get the default signature from "func" */
	jit_type_t signature = func->signature;

	/* Determine the nesting relationship with the current function */
	if(func->nested_parent) {
		printf("JIT can't create nested tail calls\n");
		return 0;
	} else {
		is_nested = 0;
		nesting_level = 0;
	...
	/* Create the instructions to push the parameters onto the stack */
	if(!create_call_setup_insns
			(func, func, signature, new_args, num_args,
			 is_nested, nesting_level, &return_value, flags))
	{
		return 0;
	}
...
//		label_end = jit_label_undefined;
		if(!jit_insn_branch(func, entry_point))
		{
			printf("%p JIT TAIL CALL: CAN'T BRANCH BACK\n", func);
			return 0;
		}
/*		if(!jit_insn_label(func, &entry_point))
		{
			printf("%p JIT TAIL CALL: CAN'T CREATE LABEL ENTRY POINT\n", func);
			return 0;
		}
		if(!jit_insn_label(func, &label_end))
		{
			printf("%p JIT TAIL CALL: CAN'T CREATE LABEL END POINT\n", func);
			return 0;
		}
		if(!jit_insn_move_blocks_to_start(func, entry_point, label_end))
		{
			printf("%p JIT TAIL CALL: CAN'T MOVE ENTRY POINT TO START OF FN\n", func);
			return 0;
		}
*/	}
